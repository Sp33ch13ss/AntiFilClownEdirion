...
void graph_menu() {
	...
	while (1) {
		int optionn = 0;
		printf("Menyu:\n");
		printf("%d. Vyvesti graf.\n", ++optionn);
		printf("%d. Vvesti graf.\n", ++optionn);
		printf("%d. Najti cikly v grafe.\n", ++optionn);
		printf("%d. Proizvesti topologicheskuyu sortirovku grafa.\n", ++optionn);
		printf("%d. Najti hromaticheskoe chislo grafa (neorientirovannogo).\n", ++optionn);
		printf("%d. Najti maksimal'nyj potok/minimal'nyj razrez (odno i to zhe chislo).\n", ++optionn);
		printf("%d. Najti kratchajshee rasstoyanie ot v0 do naibolee udalennoj vershiny.\n", ++optionn);
		printf("%d. Vyhod.\n", ++optionn);
		int option = 0;
		scanf("%d", &option);
...
void graph_not_dejkstra(struct graph* g) {
	printf("Vvedite nomer iznachal'noj vershiny\n");
	...
	free(visited);
	printf("Maksimal'noe rasstoyanie %llu do vershiny %llu\n", maxflow, maxflown);
}
...
void graph_find_chrome(struct graph* g) {
	...
	printf("Raskrasheno %llu cvetami\n", ncolors);
	printf("Cveta:\n");
	for (unsigned long long i = 0; i < g->nnodes; i++)
		printf("%d ", colors[i]);
	printf("\n");
...
void graph_top_sort(struct graph* g) {
	char flag = 0;
	if (!g)return;
	printf("Otsortirovannye vershiny:\n");
	char* marks = NULL;
	...
	while (nmarks != g->nnodes) {
		...
		if (nmarks == oldnmarks)printf("V grafe est' cikly, sortirovka nevozmozhna\n");
	}
	printf("\n");
	free(marks);
}
...
struct graph* graph_input() {
	...
	printf("Vvedite kol-vo vershin v grafe\n");
	scanf("%llu", &n);
	struct graph* g = graph_new(n);
	printf("Vvedite sposob vvoda grafa. 1 - po rebram, 2 - matrica smezhnosti:\n");
	int way = 0;
	scanf("%d", &way);
	if (way == 1) {
		printf("Graf orientirovannyj? 1-da, 2-net\n");
		int direct = 0;
		scanf("%d", &direct);
		printf("Vvodite rebra grafa po odnomu na stroku v formate trekh chisel(ot, do, ves) razdelennyh probelom. Po okonchanii vvoda vvedite -1. Vershiny numeruyutsya s 0\n");
